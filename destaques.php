...
<body style="background-color: #343440">
    <?php
        session_start();
        include'conexao.php';
        include'navbar.php';
        ...
        $consulta = $cn->query('select cod_guitarra, nome, preco, nome_imagem, qt_estoque from vw_guitarra where destaques_semana = \'S\';');
    ?>

    <h1 class="jumbotron text-center" style="background-image: url(img/destaques.png); background-position: left bottom; color: #fff; font-size: 4em; font-weight: bold;">Destaques</h1>
        ...
        <?php while($exibe = $consulta->fetch(PDO::FETCH_ASSOC)) { ?>
            <div class="col-sm-3">
                <div style="border-radius: 24px; background: #343440; box-shadow:  9px 9px 29px #202028, -9px -9px 29px #484858; padding: 1em; margin: .25em; margin-bottom: 2em;">
                    ...
                    <div><h4 style="color: #f5be01">R$<?php echo number_format($exibe['preco'], 2, ',', '.'); ?>&nbsp<span class="label label-basic" style="color: #3a3a4b; background-color: #ffffff;"><?php echo "Quantidade:".$exibe['qt_estoque'] ?></span></h4></div>

                    <div class="text-center" style="display: flex; gap: .5em; flex-wrap: wrap;">
                        <a href="detalhes.php?cd=<?php echo $exibe['cod_guitarra'];?>">
                            <button class="btn btn-lg btn-default" style="width: 6em; height: 3em;">
                                <span>Ver mais</span>
                            </button>
                        </a>
